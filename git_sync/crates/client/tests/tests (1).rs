...
#[test]
fn test_post_merge_hook() -> Result<(), Box<Error>> {
    ::sync_server::init_logging();
...
fn create_repo_test<F>(test_body: F) -> Result<(), Box<Error>>
...
#[cfg(windows)]
fn install_all_hooks(repo: &Repository) -> Result<(), Box<Error>> {
...
#[cfg(windows)]
fn install_hook<P: AsRef<Path>>(hooks_dir: P, hook_name: &str) -> Result<(), Box<Error>> {
...
#[must_use]
fn git_cmd(repo: &Repository, arguments: &[&str]) -> Result<(), Box<Error>> {
...
fn change_and_commit<P: AsRef<Path>>(repo: &Repository, changes: &[(P, &str)]) -> Result<(), Box<Error>> {
