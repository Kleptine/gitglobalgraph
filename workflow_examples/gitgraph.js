...
(function () {
  ...
  GitGraph.prototype.message = function(message) {
    ...
    this.messages.push({
      parent: this,
      message: message,
      y: -this.commitOffsetY,
    });

    this.commitOffsetY += this.template.commit.spacingY ;
  }
  ...
  GitGraph.prototype.render = function () {
    ...
    if (this.template.commit.message.display) {
      unscaledResolution.x += 450;
    } else {
      unscaledResolution.x += 100;
    }
    ...
    for (var k = 0; k < this.messages.length; k++) {
      ...
      var commitOffsetLeft = (this.columnMax + 1) * this.template.branch.spacingX + commitOffsetForTags;
      this.context.font= 'bold 11pt monospace';
      this.context.fillText(message.message, commitOffsetLeft, message.y);
    }
  ...
  Branch.prototype.merge = function (target, commitOptions) {
    ...
    commitOptions.mergeTargetParentCommit = targetBranchParentCommit;
  ...
  function Commit(options) {
    ...
    this.sha1 = options.sha1 || Math.random(0,100).toString(16).substring(3, 10);
    ...
    this.filesChanged = options.filesChanged;
    this.mergeTargetParentCommit = options.mergeTargetParentCommit;
  ...
  function _drawTextBG(context, x, y, text, color, font, angle, useStroke) {
    ...
    context.fillText(text, 0, height / 2 - 2);
    context.restore();
  }
